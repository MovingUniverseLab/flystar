...
def weighted_mean(df, x, xe, all_frames):
	...
	for i in range(len(array_x)):
        ...
        # For stars that have at least one match, calculate
		...
		if mask.sum() > 1: 
            ...
            # For stars that have no match, just append their original values and errors.
...
def stitch(name_starlist,name_ref, name_initial_ref, corr_thresh_starlist=0.8, corr_thresh_ref=0.8):
	...
	# Table -> dataframe -> Table, which lets us avoid the following error:
    #     'MaskedColumn' object has no attribute '_mask'
	...
	_,_,_,trans = examples.align_starlists(starlist_for_align, ref_for_align, order=2, dr_tol=1,
                                               N_loop=15, outFile='/u/dkim/Documents/outTrans.txt')
	...
	idx_starlist_transformed_matched, idx_ref_matched, dr, dm = match.match(starlist_transformed['x'],
                                                                            starlist_transformed['y'],
                                                                            starlist_transformed['m'],
                                                                            ref['x'], ref['y'], ref['m'],
                                                                            dr_tol=1)
    ...
    # If the input starlist has been already used and exists within the master reference,
	...
	new_ref.write('{0}.lis'.format(name_new_ref), format='ascii.commented_header',
                      header_start=-1, overwrite=True)
...
# FROM JLU: While multiplying this by N_iter is a clever way to code
# up a longer loop, we actually want to be explicit about the number
# of iterations that should be done. In other words, this needs to be
# an input parameter into the algorithm. We will need to be able to check
# the improvements after each full N_iter.
