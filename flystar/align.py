...
class MosaicSelfRef(object):
    ...
    def match_and_transform(self, ref_mag_lim, dr_tol, dm_tol, outlier_tol, trans_args):
        ...
        for ii in range(len(self.star_lists)):
            ...
            if isinstance(trans, (flystar.transforms.UVIS_CTE_trans, 
                                  flystar.transforms.UVIS_CTE_ACS_ISR_0704_trans,
                                  flystar.transforms.UVIS_CTE_2_trans)):
                if self.mag_trans:
            ...
            if self.trans_class in [flystar.transforms.UVIS_CTE_trans, 
                                    flystar.transforms.UVIS_CTE_ACS_ISR_0704_trans,
                                    flystar.transforms.UVIS_CTE_2_trans]:
                # Use this as a guess
...
def trans_initial_guess(ref_list, star_list, trans_args, mode='miracle',
    ...
    if ('order' in trans_args) and (trans_args['order'] == 0):
        ...
        order = 1
    if trans_class in [flystar.transforms.UVIS_CTE_trans, 
                       flystar.transforms.UVIS_CTE_ACS_ISR_0704_trans,
                       flystar.transforms.UVIS_CTE_2_trans]:
